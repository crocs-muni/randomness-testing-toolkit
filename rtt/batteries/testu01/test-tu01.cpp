...
std::unique_ptr<Test> Test::getInstance(int testIndex ,
    ...
    std::shared_ptr<Test> t (new Test(testIndex , container));

    t->logicName =
            std::get<0>(TestConstants::getTu01TestData(t->battery , t->testIndex));

    uint varCount = t->batteryConfiguration->getTestVariantsCount(
                        t->battery, testIndex);
